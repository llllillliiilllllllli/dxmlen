/// <summary>
/// Ranking search engine results by LightGBM learning algorithm.
/// Source: https://en.wikipedia.org/wiki/Learning_to_rank
/// Source: https://www.microsoft.com/en-us/research/wp-content/uploads/2016/02/MSR-TR-2010-82.pdf
/// Source: https://www.microsoft.com/en-us/research/wp-content/uploads/2016/02/lambdarank.pdf
/// Source: https://learn.microsoft.com/en-us/dotnet/api/microsoft.ml.trainers.lightgbm?view=ml-dotnet
/// Source: https://www.microsoft.com/en-us/research/project/mslr/
/// Source: https://github.com/dotnet/machinelearning/tree/main/src/Microsoft.ML.LightGbm
/// Source: https://github.com/dotnet/machinelearning-samples/tree/main/samples/csharp/getting-started/Ranking_Web
/// </summary>
...
namespace DxMLEngine.Features.Ranking
{
    [Feature]
    internal class SearchEngineRanking
    {
        ...
        public class SearchResultPrediction : SearchResult
        {
        ...
        public static void CleanSearchData(string inFile)
        {
            var dataset = new List<string>();
            var lines = File.ReadAllLines(inFile, encoding: Encoding.UTF8);
            for (var i = 0; i < lines.Length; i++) 
            {
                var values = lines[i].Split(" ", StringSplitOptions.TrimEntries);
                var cleanedLines = new List<string>() { values[0] };
                for (var j = 1; j < values.Length - 1; j++)
                    cleanedLines.Add(values[j].Split(":")[1]);

                var dataRow = string.Join("\t", cleanedLines);
                Console.WriteLine(dataRow);
                
                dataset.Add(dataRow);
            }
            var outFile = $"{Path.GetDirectoryName(inFile)}\\Dataset {Path.GetFileNameWithoutExtension(inFile)} .tsv";
            File.WriteAllLines(outFile, dataset, encoding: Encoding.UTF8);
        }

        public static void RankSearchEngine(string inFileTrain, string inFileValid, string inFileTest, string outDir, string fileName)
        {
            var mlContext = new MLContext();

            var trainData = mlContext.Data.LoadFromTextFile<SearchResult>(inFileTrain);
            var validData = mlContext.Data.LoadFromTextFile<SearchResult>(inFileValid);
            var testData = mlContext.Data.LoadFromTextFile<SearchResult>(inFileTest);

            var featureNames = (
                from column in trainData.Schema.AsQueryable()
                where column.Name != nameof(SearchResult.GroupId)
                where column.Name != nameof(SearchResult.Label)
                select column.Name).ToArray();

            var pipeline = mlContext.Transforms.Concatenate(inputColumnNames: featureNames, outputColumnName: "Features")
                .Append(mlContext.Transforms.Conversion.MapValueToKey(inputColumnName: nameof(SearchResult.Label), outputColumnName: "LabelKey"))
                .Append(mlContext.Transforms.Conversion.Hash(inputColumnName: nameof(SearchResult.GroupId), outputColumnName: nameof(SearchResult.GroupId), numberOfBits: 20))
                .Append(mlContext.Ranking.Trainers.LightGbm(labelColumnName: "LabelKey", featureColumnName: "Features", rowGroupColumnName: nameof(SearchResult.GroupId)))
                .Append(mlContext.Transforms.Conversion.MapKeyToValue(inputColumnName: "LabelKey", outputColumnName: "PredictedLabel"));

            var model = pipeline.Fit(trainData);
            var predDataView = model.Transform(validData);
            var predictions = mlContext.Data.CreateEnumerable<SearchResultPrediction>(predDataView, false, ignoreMissingColumns: true);
            var metrics = mlContext.Ranking.Evaluate(predDataView);

            Log.Info("Ranking Search Engine Results with LightGbm");
            foreach (var prediction in predictions)
            {
                Console.WriteLine($"ActualLabel     : {prediction.Label}");
                Console.WriteLine($"PredictedLabel  : {prediction.PredictedLabel}");
                Console.WriteLine($"Score           : {prediction.Score}");
                Console.WriteLine($"Features        : [ {string.Join(", ", prediction.Features!)} ]");
            }

            Log.Info("LightGbm Ranking Metrics");
            Console.WriteLine($"DiscoutedCummulativeGains: [ {string.Join(", ", metrics.DiscountedCumulativeGains)} ]");
            Console.WriteLine($"DiscoutedCummulativeGains: [ {string.Join(", ", metrics.NormalizedDiscountedCumulativeGains)} ]");            
            Console.Write("\nPress key to continue..."); Console.ReadKey();

            var trainDataEnum = mlContext.Data.CreateEnumerable<SearchResult>(trainData, false);
            var validDataEnum = mlContext.Data.CreateEnumerable<SearchResult>(validData, false);
            var trainValidEnum = trainDataEnum.Concat<SearchResult>(validDataEnum);
            var trainValidationData = mlContext.Data.LoadFromEnumerable<SearchResult>(trainValidEnum);

            var revisedModel = pipeline.Fit(trainValidationData);
            var revisedPredDataView = revisedModel.Transform(testData);
            var revisedPredictions = mlContext.Data.CreateEnumerable<SearchResultPrediction>(revisedPredDataView, false);
            var revisedMetrics = mlContext.Ranking.Evaluate(revisedPredDataView);

            Log.Info("Ranking Search Engine Results with LightGbm");
            foreach (var prediction in revisedPredictions)
            {
                Console.WriteLine($"ActualLabel    :{prediction.Label}");
                Console.WriteLine($"PredictedLabel :{prediction.PredictedLabel}");
                Console.WriteLine($"PredictedLabel :{prediction.Score}");
                Console.WriteLine($"Features        : [ {string.Join(", ", prediction.Features!)} ]");
            }

            Log.Info("Revised LightGbm Ranking Metrics");
            Console.WriteLine($"DiscoutedCummulativeGains: [ {string.Join(", ", revisedMetrics.DiscountedCumulativeGains)} ]");
            Console.WriteLine($"DiscoutedCummulativeGains: [ {string.Join(", ", revisedMetrics.NormalizedDiscountedCumulativeGains)} ]");
            Console.Write("\nPress key to continue..."); Console.ReadKey();

            var path = $"{outDir}\\Model @{fileName} .zip";
            mlContext.Model.Save(revisedModel, testData.Schema, path);
        }
    }
}
